...
type dsharp =
    ...
    static member empty(shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Empty(shape|>Shape.constant, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member empty(length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Empty(Shape.constant [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member zeros(shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Zeros(shape|>Shape.constant, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member zeros(length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Zeros(Shape.constant [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member ones(shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Ones(shape|>Shape.constant, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member ones(length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Ones(Shape.constant [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member full(shape:seq<int>, value:obj, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Full(shape|>Shape.constant, value, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member rand(shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Random(Shape.constant shape, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member rand(length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.Random(Shape.constant [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member randn(shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.RandomNormal(Shape.constant shape, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member randn(length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.RandomNormal(Shape.constant [|length|], ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member randint(low:int, high:int, shape:seq<int>, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.RandomInt(Shape.constant shape, low, high, ?dtype=dtype, ?device=device, ?backend=backend))
    ...
    static member randint(low:int, high:int, length:int, ?dtype:Dtype, ?device:Device, ?backend:Backend) =
        Tensor(RawTensor.RandomInt(Shape.constant [|length|], low, high, ?dtype=dtype, ?device=device, ?backend=backend))
